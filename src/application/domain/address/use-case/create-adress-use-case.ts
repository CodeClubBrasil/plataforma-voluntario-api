import { Address, AdressData, State } from "src/domain";
import { AdressRepository } from "../repository/address";
...
export class CreateAdressUseCase {
    constructor(
        private adressRepostiory: AdressRepository
    ) { }
    ...
    async execute(request: AddressRequest): Promise<AdressResponse> {
        ...
        console.log("USE CASE CHECK IF DATA IS COMING" + zipCode, address, city, state);
